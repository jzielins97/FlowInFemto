...
void CorrFctnDirectYlm::AddRealPair(double qout, double qside, double qlong, double weight)
{
  ...
  SpherHarmonics::YlmUpToL(elsi[maxjm-1], qout, qside, qlong, ylmbuffer);
  for (int ilmv=0; ilmv<maxjm; ilmv++)
    ylmbuffer[ilmv] *= weight;

  for (int ilm=0; ilm<maxjm; ilm++) {
    //    ylmbuffer[ilm] = SpherHarmonics::Ylm(elsi[ilm], emsi[ilm], qout, qside, qlong);

    if (emsi[ilm] < 0) continue;

    numsreal[ilm]->Fill(kv, real(ylmbuffer[ilm]));
    if (emsi[ilm]  ==  0) continue;
    numsimag[ilm]->Fill(kv, -imag(ylmbuffer[ilm]));
  ...
  //  int tabshift = nqbin*maxjm*maxjm*4;
  
  int b1; 

  if (nqbin < binctn->GetNbinsX()) {
    b1 = nqbin * maxjm * maxjm * 4;
    for (int ilmzero=0; ilmzero<maxjm; ilmzero++) {
      if (emsi[ilmzero] < 0) { b1 += 4*maxjm; continue; }
      for (int ilmprim=0; ilmprim<ilmzero+1; ilmprim++) {
	if (emsi[ilmprim] <0) { continue; }
	

// 	covmnum[GetBin(nqbin, ilmzero, 0, ilmprim, 0)] += real(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
// 	covmnum[GetBin(nqbin, ilmzero, 0, ilmprim, 1)] += real(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
// 	covmnum[GetBin(nqbin, ilmzero, 1, ilmprim, 0)] += -imag(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
// 	covmnum[GetBin(nqbin, ilmzero, 1, ilmprim, 1)] += -imag(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	if (emsi[ilmzero]) {
	  if (emsi[ilmprim]) {
	    covmnum[b1 +                 ilmprim*2    ] += real(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    covmnum[b1 +                 ilmprim*2 + 1] += real(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	    covmnum[b1 + maxjm + maxjm + ilmprim*2    ] += -imag(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    covmnum[b1 + maxjm + maxjm + ilmprim*2 + 1] += -imag(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	  }
	  else {
	    covmnum[b1 +                 ilmprim*2    ] += real(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    //	    covmnum[b1 +                 ilmprim*2 + 1] += 0;
	    covmnum[b1 + maxjm + maxjm + ilmprim*2    ] += -imag(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    //	    coavmnum[b1 + maxjm + maxjm + ilmprim*2 + 1] += 0;
	  }
	}
	else {
	  if (emsi[ilmprim]) {
	    ...
	    covmnum[b1 +                 ilmprim*2 + 1] += real(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	    //	    covmnum[b1 + maxjm + maxjm + ilmprim*2    ] += -imag(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    //	    coavmnum[b1 + maxjm + maxjm + ilmprim*2 + 1] += -imag(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	    
	  }
	  else {
	    covmnum[b1 +                 ilmprim*2    ] += real(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    //	    covmnum[b1 +                 ilmprim*2 + 1] += real(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	    //	    coavmnum[b1 + maxjm + maxjm + ilmprim*2    ] += -imag(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    //	    covmnum[b1 + maxjm + maxjm + ilmprim*2 + 1] += -imag(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	  }
	}
      }
...
{
  double kv = sqrt(qout*qout + qside*qside + qlong*qlong);
  int nqbin = binctd->GetXaxis()->FindFixBin(kv) - 1;
  int maxjm = GetMaxJM();
  ...
  if (nqbin < binctn->GetNbinsX()) {
	...
	if (emsi[ilmzero]) {
	  if (emsi[ilmprim]) {
	    covmden[b1 +                 ilmprim*2    ] += real(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    covmden[b1 +                 ilmprim*2 + 1] += real(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
	    covmden[b1 + maxjm + maxjm + ilmprim*2    ] += -imag(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    covmden[b1 + maxjm + maxjm + ilmprim*2 + 1] += -imag(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);	    
	  }
	  else {
	    covmden[b1 +                 ilmprim*2    ] += real(ylmbuffer[ilmzero])*real(ylmbuffer[ilmprim]);
	    //	    covmden[b1 +                 ilmprim*2 + 1] += real(ylmbuffer[ilmzero])*-imag(ylmbuffer[ilmprim]);
...
void CorrFctnDirectYlm::FillCovariances(){
  // This procedure should be called only once
  // immediately before saving the correlation function
  // It fills in missing information, which is not
  ...
  if (covnum) 
	...
	for (int ilmp=0; ilmp<GetMaxJM()*2; ilmp++)
	  if (ilmp>ilmz)
	    ...
	    covnum->SetBinContent(ibin, ilmz+1, ilmp+1, covmnum[GetBin(ibin-1, ilmp/2, ilmp%2, ilmz/2, ilmz%2)]);
  ...
  if (covden)
	...
	for (int ilmp=0; ilmp<GetMaxJM()*2; ilmp++)
	  if (ilmp>ilmz)
	    ...
	    covden->SetBinContent(ibin, ilmz+1, ilmp+1, covmden[GetBin(ibin-1, ilmp/2, ilmp%2, ilmz/2, ilmz%2)]);

  int hfrom, hdest;
  int mre, mim;

  for (int il=0; il<=fMaxL; il++) {
    mre = -1;
    mim = 1;
    for (int im=-1; im>(-(il+1)); im--) {
      hfrom = il*il + il - im;
      hdest = il*il + il + im;

      cout << "Reading " << il << " "<< im << " "<< hfrom << " " << hdest << endl;
      cout << " From To " << numsreal[hfrom]->GetName() << " " << numsreal[hdest]->GetName() << endl;

      for (int ib=1; ib<=numsreal[hfrom]->GetNbinsX(); ib++) {
	cout << "Bin " << ib << endl;
	
	numsreal[hdest]->SetBinContent(ib, mre*numsreal[hfrom]->GetBinContent(ib));
	densreal[hdest]->SetBinContent(ib, mre*densreal[hfrom]->GetBinContent(ib));

	numsimag[hdest]->SetBinContent(ib, mim*numsimag[hfrom]->GetBinContent(ib));
	densimag[hdest]->SetBinContent(ib, mim*densimag[hfrom]->GetBinContent(ib));

	numsreal[hdest]->SetBinError(ib, mre*numsreal[hfrom]->GetBinError(ib));
	densreal[hdest]->SetBinError(ib, mre*densreal[hfrom]->GetBinError(ib));

	numsimag[hdest]->SetBinError(ib, mim*numsimag[hfrom]->GetBinError(ib));
	densimag[hdest]->SetBinError(ib, mim*densimag[hfrom]->GetBinError(ib));
      }
...
void CorrFctnDirectYlm::AddToList(TList *fOutput)
{
  for (int ilm=0; ilm<maxjm; ilm++) {
